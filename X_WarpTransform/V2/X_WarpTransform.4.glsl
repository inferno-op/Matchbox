...
uniform sampler2D adsk_results_pass2, adsk_results_pass1, adsk_results_pass3, anim_layer;
...
vec4 warp(int texture_source, float multiplier) {
	...
	if (texture_source == 0) {
   		...
   		warped = texture2D(adsk_results_pass3, coords);
	}
...
void main()
{
	...
	vec4 tmp = texture2D(adsk_results_pass2, st);

	for (int i = 0; i < 8; i++) {
		if (tmp != vec4(0.0)) {
			tmp *= texture2D(adsk_results_pass2, st + around[i] / vec2( adsk_result_w, adsk_result_h));
		}
	}


    vec4 premult = front * (1.0 - tmp);
    vec4 anim_layer = texture2D(anim_layer, st);
	...
	if (repeat_texture) {
		warped = warp(source, matte.r);
	} else {
		if (isInTex(get_coords(matte.r))) {
			warped = warp(source, matte.r);
		}
	}
