...
void main(void)
{
	...
	if (isInTex(st)) {
			while (x <= adsk_result_w) {
				...
				tmp1 += texture2DLod(adsk_results_pass1, gl_FragCoord.xy / vec2(x, adsk_result_h), lod);
		...
		while (y <= adsk_result_h) {
			tmp2 += texture2DLod(adsk_results_pass1, gl_FragCoord.xy / vec2(adsk_result_w, y), lod);
